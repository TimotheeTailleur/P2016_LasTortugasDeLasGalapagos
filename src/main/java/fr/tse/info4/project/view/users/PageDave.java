...
public class PageDave extends TabReference {
	...
	/**
	 * Dave Tab Constructor <br>
	 * Displays method results for each functionality of the Dave User Story
	 */
	public PageDave() {
		super();
		...
		if (forceUpdate)
			...
			dave = (new UserFactory()).newDave().withNbUsers(nbUsers).withRefreshRate(refreshRate).withoutForceUpdate()
					.get();
		...
		option1.addMouseListener(new MouseListener() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				if (isList == false) { // if one tag was written, launch
										// function 1 and display results

					TagScore topTag = dave.getTopTag(tagName.getText());
					str = StringEscapeUtils
							.unescapeHtml((new ApiManager()).getUserNAme((int) topTag.getUser().getUserId()))
							+ " avec un score de " + topTag.getScore();
			...
			public void actionPerformed(ActionEvent event) {
				...
				validation.addMouseListener(new MouseListener() {
					...
					public void mouseReleased(MouseEvent e) {
						...
						if (yes.isSelected()) {
							dave = (new UserFactory()).newDave().withNbUsers(nbUsers)
									.withRefreshRate(refreshRateTopAnswerers).withForceUpdate().get();
						} else {
							dave = (new UserFactory()).newDave().withNbUsers(nbUsers)
									.withRefreshRate(refreshRateTopAnswerers).withoutForceUpdate().get();
						}
				...
				reinitialiser.addMouseListener(new MouseListener() {
					...
					public void mouseReleased(MouseEvent e) {
						...
						dave = (new UserFactory()).newDave().withNbUsers(10).withRefreshRate(24).withoutForceUpdate()
								.get();
						parametre.dispose();
						try {
							PropertiesConfiguration config = new PropertiesConfiguration(PARAMETERS_PATH);
